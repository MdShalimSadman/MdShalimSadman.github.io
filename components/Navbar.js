...
const Navbar = () => {
  ...
  return (
    <header className="text-gray-600 body-font fixed w-full !z-50 mt-6">
      ...
      <div
        className={`fixed top-0 right-0 h-full w-64 bg-gray-900 bg-opacity-95 backdrop-blur-md transform ${
          drawerOpen ? "translate-x-0" : "translate-x-full"
        } transition-transform duration-300 ease-in-out z-50`}
      >
        <div className="flex justify-end p-4">
          <button onClick={() => setDrawerOpen(false)} className="text-white text-3xl">
            <FiX />
          </button>
        </div>
        <nav className="flex flex-col mt-4 space-y-4 text-white px-6">
