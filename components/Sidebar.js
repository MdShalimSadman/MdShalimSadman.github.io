...
const Sidebar = () => {
  ...
  return (
    <div className="fixed top-1/2 transform -translate-y-1/2 right-0 h-55 w-10 bg-gray-900 flex flex-col items-center justify-center rounded-md ">
      {socialMediaLinks.map((socialMedia, index) => (
        <a
          ...
          className="text-white hover:text-blue-300 transition-colors duration-300 sm:mb-2 sm:mt-2"
         
        >
          {socialMedia.icon}
        </a>
      ))}
      <div className="pb-3 mb-4 sm:mb-0 sm:pb-0"></div>
    </div>
  );
};
